...
$stmt = $pdo->prepare("    
    ...
    FROM (
        ...
        WHERE id = ? OR sender_id = ? OR receiver_id = ?  
    ) t
...
$stmt->execute([$transaction_id, $transaction_id, $transaction_id, $transaction_id]);
...
<body>
    <div class="container">
        ...
        <?php if ($transaction['type'] == 'topup' || $transaction['type'] == 'withdraw'): ?>
            ...
            <p><strong>Status:</strong> <span class="status <?= strtolower($transaction['status']) ?>">
                <?= ucfirst(htmlspecialchars($transaction['status'])) ?></span></p>
            <p><strong>Jumlah:</strong> Rp <?= number_format($transaction['jumlah'], 2) ?></p>
            <p><strong>Tanggal:</strong> <?= htmlspecialchars($transaction['created_at']) ?></p>
        <?php endif; ?>
        ...
        <a href="view_transactions.php" class="btn">Kembali</a>
